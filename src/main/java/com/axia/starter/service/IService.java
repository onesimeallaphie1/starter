...
public interface IService<E, D, ID> {
    ...
    Page<D> paginate(Consumer<SpecificationBuilder.GroupBuilder<E>> builderConsumer, CustomPageRequest pageRequest);
    byte[] export(Consumer<SpecificationBuilder.GroupBuilder<E>> builderConsumer);
    List<D> list(ConditionGroup conditionGroup);
    Page<D> paginate(ConditionGroup conditionGroup, CustomPageRequest pageRequest);
